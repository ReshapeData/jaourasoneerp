   ...
   #更新功能处理 begin----
    ...
    for (i in 1:batch) {
       ...
       sql_sl1=sprintf("select a.FSETTLENO ,a.F_KD_SETTLENO ,a.F_KD_OUTHUBDATE ,a.F_KD_OUTAMOUNT from T_AR_RECEIVEBILLENTRY a inner join T_AR_RECEIVEBILL b  on  a.FID = b.fid where b.FBILLNO='%s' and a.FSEQ ='%s'",FBillno,FSeq)
        ...
        print(yy[3])
        print(yy[4])
        # if(yy[1]==(item$结算单号)) print("是偶数")
        # 
        # print("执行完毕！")
        
        # tsda::db_writeTable2(token = 'F91CF3E3-8962-47F2-823F-C5CCAAFC66CA',table_name = 'ar_rece_update_info',r_object = item,append = TRUE)
        
        #更新语句
        sql_up=sprintf("update a set a.FSETTLENO ='%s', a.F_KD_SETTLENO ='%s' ,a.F_KD_OUTHUBDATE = '%s', a.F_KD_OUTAMOUNT ='%s'from T_AR_RECEIVEBILLENTRY a inner join T_AR_RECEIVEBILL b on  a.FID = b.fid where b.FBILLNO='%s' and a.FSEQ ='%s'",FSettleNo,F_KD_SETTLENO,F_KD_OUTHUBDATE,F_KD_OUTAMOUNT,FBillno,FSeq)
        print(sql_up)
        
        # tsda::sql_update2(token = 'F91CF3E3-8962-47F2-823F-C5CCAAFC66CA',sql_str = sql_up)
         }
